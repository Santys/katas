...
function rot13(message){
  ...
  for(let i = 0; i < message.length; i++){
    index = message[i].charCodeAt()
    let code = 0
    if(index < 64){
      code = index
    } else if(index <= 90) {
      (index + 13) <= 90 ? code = index + 13 : code = (index+13)%90 + 64
    } else if(index >= 97){
      (index + 13) <= 122 ? code = index + 13 : code = (index+13)%122 + 96
    }
    ciphered.push(String.fromCharCode(code))
  }
